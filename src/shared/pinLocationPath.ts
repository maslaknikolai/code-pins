...
export function buildPinLocationPath(path: string, line: number, character: number): string {
	return `${path}:${line}:${character}`;
}

/** The path may contain colons, so the numbers are taken from the end. */
export function parsePinLocationPath(symbolPath: string): { line: number; character: number } {

...
suite('buildPinsTree', () => {
	...
	test('two pins on the same line with nothing deeper both keep their line', () => {
		const a: Pin = {
			id: 'a',
			locationKey: 'src/a.ts:3:10',
			symbolName: 'f',
			lines: [{ line: 3, text: 'const x = f(g)' }],
		};
		const b: Pin = {
			id: 'b',
			locationKey: 'src/a.ts:3:12',
			symbolName: 'g',
			lines: [{ line: 3, text: 'const x = f(g)' }],
		};

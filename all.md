...
2. Для каждого такого пина восстанавливаем позицию: парсим его `symbolLocationPath` (`packages/campaigns-ui/src/components/tables/ListsTable/ListPopupMenu.tsx:22:11` → файл, строка 22, колонка 11), открываем документ.
...
```ts
symbolLocationPath: string      // где сам symbol:      "packages/campaigns-ui/src/components/tables/ListsTable/ListPopupMenu.tsx:22:11"
...
const pinA: Pin = {
	id: 'a1…',
	symbolName: 'addTab',
	symbolLocationPath:   'packages/campaigns-state-page/src/events.ts:7:4',
...
const pinB: Pin = {
	id: 'b2…',
	symbolName: 'addTab',
	symbolLocationPath:   'packages/campaigns-ui/src/components/tables/ListsTable/ListPopupMenu.tsx:22:11',
...
const pinC: Pin = {
	id: 'c3…',
	symbolName: 'addTab',
	symbolLocationPath:   'packages/campaigns-ui/src/components/tables/ListsTable/ListPopupMenu.tsx:51:16',
...
- **Объявление**: `symbolDefinitionPath === symbolLocationPath` (синяя полоска слева).
- **`Edge`**: мой `symbolDefinitionPath` равен чьему-то `symbolLocationPath` в другом узле. Цепочка C → B → A.
- **Один и тот же `symbol`** (подсветка при выборе): любые два ключа пересеклись (`checkIsSameSymbol`).
- **Дубликат**: одинаковый `symbolLocationPath` — этот `symbol` уже запинен.
...
Строки храним **сырыми** (с отступами): колонка из `symbolLocationPath` указывает прямо в текст — отдельный `symbolRange` не нужен.

## Модель данных

```ts
Pin      = { id, symbolLocationPath, symbolDefinitionPath?, symbolName, lines: PinLine[] }

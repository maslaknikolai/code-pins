# Code Pins — как всё работает

Расширение VS Code: визуальная карта кода. Ставишь курсор на symbol, жмёшь хоткей — symbol попадает на карту как «пин». Пины одного файла живут в одном узле, между узлами рисуются стрелки «кто на кого ссылается».

**Symbol** — именованная сущность кода: переменная, функция, метод, класс, компонент. В примере ниже symbol — это `addTab`. Термин из VS Code / language server: они оперируют именно symbol'ами (definition symbol'а, дерево symbol'ов файла).

## Архитектура

Два мира, общаются сообщениями:

- **Extension host** (`src/extension/`) — Node-часть. Команды, language server, хранение данных карты, сохранение.
- **Webview** (`src/webview/`) — React Flow.
- **Общее** (`src/shared/`) — типы и ключи (`types.ts`, `locationKey.ts`).
...
Иногда ответ пустой: language server ещё индексирует проект, или для языка нет провайдера. Тогда пин создаётся без `definitionKey` — он просто не участвует в стрелках.

Это чинит `retryUnresolvedDefinitions`. Он запускается при последующих добавлениях пинов и при открытии карты, в фоне. Алгоритм:
...
- **Стрелка**: мой `definitionKey` равен чьему-то `locationKey` в другом узле. Цепочка C → B → A.
...
FileNode = { filePath, x, y, pins: Pin[] }       // узел карты = файл; filePath = id узла
CodePinsFile = { version: 2, fileNodes }         // формат сохранения
```

Позиция узла — на `FileNode`, у пина позиции нет. Стрелки не хранятся — выводятся из ключей.
...
Клик по подсвеченному symbol'у → в атом `selectedSymbolAtom` кладутся ключи пина. Каждый `PinLineView` сравнивает себя с выбранным через `checkIsSameSymbol` — совпал → стиль выделения. `App` тем же сравнением красит стрелки (линия и наконечник цветом focusBorder). Повторный клик — сброс.

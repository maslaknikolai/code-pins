...
`node`'ы с общими директориями обводятся серыми рамками, вложенно: каждая директория, в поддереве которой ≥2 `node`'ов, даёт свою рамку — `packages/campaigns-state-page/src` своя, вокруг неё общая `packages`. Над левым верхним углом рамки — общий префикс. `groupFileNodes` строит trie по сегментам пути (цепочки директорий без развилок схлопываются в один префикс, группы из одного `node` не рисуются). `GroupOutlines` считает bounding box по позициям и `measured`-размерам (паддинг растёт с уровнем вложенности, чтобы рамки не слипались) и рисует через `ViewportPortal` — div живёт в координатах канваса, двигается с pan/zoom сам. Позиции берутся из `flowNodesAtom`, поэтому рамки следуют за drag'ом без доп. логики.
...
- Команды: Save/Open/Clear (`.json`, формат `CodePinsFile`). `cmd+s` работает при фокусе на панели.
